...
using MiniLedger.Application.Common.Responses;
using MiniLedger.Application.DTOs.Accounts;
using MiniLedger.Application.Interfaces;
using MiniLedger.Domain.Entities;

namespace MiniLedger.Application.Features.Accounts.Queries.GetAccounts;
    ...
    public async Task<PagedResponse<List<AccountDto>>> Handle(GetAccountsQuery request, CancellationToken cancellationToken)
    {
        ...
        if (!string.IsNullOrWhiteSpace(request.Search))
        {
            query = query.Where(x =>
                ...
                x.Code.Contains(request.Search, StringComparison.OrdinalIgnoreCase));
        }
